    ...
    <svg viewBox="0 0 1000 1000">
      <defs>
        ...
        <pattern v-for="item in ptnList" :id="item.id" :key="item.id" patternUnits="userSpaceOnUse" :width="item.width" :height="item.height">
          <image :xlink:href="getImgUrl(item.src)" x="0" y="0" :width="item.zoom"/>
        </pattern>
         <!-- filter -->
      ...
      <!-- main image -->
      <image class="main" xlink:href="../assets/hoodie1.jpg" x="-19"/>
      <!-- patterned material -->
      <path v-for="(item,index) in objList" :key="item.id" :id="item.id" class="part" :fill="'url(#' + item.ptnId +')'" filter="url(#filter1)" :d="item.d" v-on:click="setSelectedObject(index)"/>
    </svg>
    <!-- pattern list panel -->
